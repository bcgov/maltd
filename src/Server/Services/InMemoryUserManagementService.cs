...
public class InMemoryUserManagementService : IUserManagementService
{
    ...
    public Task<Result<List<ProjectResourceStatus>>> AddUserToProjectAsync(User user, ProjectConfiguration project, CancellationToken cancellationToken)
    {
        ...
        return Task.FromResult(Result.Ok(statuses));
    }
    ...
    public Task<Result<List<ProjectResourceStatus>>> RemoveUserFromProjectAsync(User user, ProjectConfiguration project, CancellationToken cancellationToken)
    {
        if (user == null) throw new ArgumentNullException(nameof(user));
        if (project == null) throw new ArgumentNullException(nameof(project));
    ...
    public Task<Result<List<Project>>> GetProjectsForUserAsync(User user, CancellationToken cancellationToken)
    {
        if (user == null) throw new ArgumentNullException(nameof(user));
        ...
        return Task.FromResult(Result.Ok(projects.Select(_ => new Project(_.Name)).ToList()));
    }
    ...
    public Task<Result<IList<UserStatus>>> GetUsersAsync(ProjectConfiguration project, ProjectResource resource, CancellationToken cancellationToken)
    {
        throw new NotImplementedException();
    }
}
